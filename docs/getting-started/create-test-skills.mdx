...
You need a running, configured container — [Quick Start](./quick-start.mdx) steps 1 to 3 — and you should
have written and run a test of your own, as in
[Write Your First Test](./write-your-first-test.mdx). This guide does not re-explain uploading tasks,
...
agent. They are unrelated to the [Waterwheel skills](../guides/agent-loops.mdx) you install into a code agent
...
- **It reads its inputs from context, not from itself.** No credential is written in the file.
  `LOGIN_URL` and `TEST_URL` are [global constants](./chain-tests-together.mdx#1-set-the-global-context)
  and the accounts are [preset context](./chain-tests-together.mdx#2-seed-a-preset-value) values — so the
...
- **It ends by cleaning up.** Releasing the snapshot keeps the run's token cost flat. See
  [Tuning](../guides/tuning.mdx#complete-verification).
...
[`SKILLS_DIR`](../reference/env-vars-reference.md) somewhere else and the same tests resolve to a different
implementation:
...
| Command | Does |
| --- | --- |
| [`load-test-skills`](../reference/usage.mdx#load-test-skills-command) | Create a skill from piped content |
| [`display-test-skills`](../reference/usage.mdx#display-test-skills-command) | List every skill, or print one's `SKILL.md` |
| [`delete-test-skills`](../reference/usage.mdx#delete-test-skills-command) | Remove your skills by name, or all of them |
| [`delete-builtin-skills`](../reference/usage.mdx#delete-builtin-skills-command) | Remove a shipped skill by exact name |
...
Everything above says a skill body costs nothing until it is used. That holds — but on Anthropic
models, *used* costs more than it first appears, and the difference is worth understanding before
you build a long suite around large skills.

Anthropic caches only a **prefix** of each request, up to an explicit cache breakpoint. Waterwheel
places that breakpoint over the material that is fixed for the whole run — the system prompt, the
instruction files, the tool definitions — and it is settled before the first step runs. A skill
loaded partway through a test arrives *after* that boundary, so its body never enters the cache. It
is billed as ordinary input tokens on every API call from the load onwards, until the test finishes
or [context compression](../guides/token-efficiency.md#enable-context-compression) discards it.

The cost is therefore not the skill body once — it is the skill body multiplied by the number of
steps that follow the load, and it grows linearly with the length of the scenario. A 2,000-token
skill pulled in at step 3 of a 40-step test is paid for roughly 37 times.

This only matters for long scenarios and substantial skill bodies; a short skill in a ten-step test
is noise. When it does matter, there are three levers:

| Lever | Use it when |
| --- | --- |
| **Run in efficiency mode** | Always. `CONTEXT_COMPRESSION=true` caps how many calls carry the skill text forward — Claude's recommended thresholds are deliberately aggressive for this reason. See the [Provider Configuration Guide](../reference/provider-guide.mdx#claude). |
| **Move the instructions to `extra-instructions.md`** | Several tests need the same prose. That file is appended to the system prompt, which sits *inside* the cached prefix, so its tokens are cached for the whole run instead of re-billed per call. See [Configuration Files](../reference/configuration-files.mdx). |
...
:::note[This is an Anthropic-specific trade-off]
The pattern above follows from how Anthropic's prefix caching works. It is not a reason to avoid
skills — the [two jobs](#what-skills-are-for) they do are still worth it, and a skill that fixes an
...
- [Manage Test Tasks](../reference/test-task-manual.md) — front matter, flow, statuses, and the authoring patterns
  a skill gets called from.
- [Chain Tests Together](./chain-tests-together.mdx) — the context layers a shared skill reads its inputs
  from.
- [Command Reference](../reference/usage.mdx) — every option for the four skill commands.
- [Token Efficiency](../guides/token-efficiency.md) — why loading instructions on demand keeps a suite cheap.
