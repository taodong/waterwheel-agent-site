...
[Write Your First Test](./write-your-first-test) covered a single, self-contained test. Real
...
You need a running, configured container — [Quick Start](./quick-start) steps 1 to 3 — and the
workflow from [Write Your First Test](./write-your-first-test) should already be familiar, since this
...
[Pass Data Between Tests](./test-task-manual#pass-data-between-tests) for the full rules.
...
[`manage-global-constants`](./usage#manage-global-constants-command).
...
[`output-context-variables`](./usage#output-context-variables-command), which prints them as flat
JSON:
...
[Dependency Check Rules](./test-task-manual#dependency-check-rules) for the complete evaluation
order.

## Next Steps

- [Create Test Skills](./create-test-skills) — extract a flow every test in the chain repeats, such
  as login, into a reusable skill instead of copying the prose.
- [Manage Test Tasks](./test-task-manual) — the full reference for front matter, flow fields,
  every test status, and the authoring patterns for multi-node suites.
- [Command Reference](./usage) — all the options for
  [`manage-global-constants`](./usage#manage-global-constants-command) and
  [`preset-context`](./usage#preset-context-command).
- [Token Efficiency](./token-efficiency) — how to group chained tests so a suite stays cheap.
- [Run through a code agent](./agent-loops) — let a code agent maintain the chain and fix what it
  breaks.
