...
Command `run-qa` executes all tests found under the `/agent/tasks` folder. It performs the following steps in order:
...
The agent requires at least one test file under `/agent/tasks`. All test files must be in text files with `.md` extention. 
...
:::info[Only file extention matters]
There is no markdown format requirement on how you structure your test file. Your tasks will be executed as long as its file extention is `.md`. 
:::
